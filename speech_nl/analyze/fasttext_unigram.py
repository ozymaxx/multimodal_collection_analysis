...
if not os.path.isfile('model_fb.bin'):
	if VALIDATION:
		call(['./fasttext supervised -input trainphrases_fb.txt -output model_fb'],shell=True)
	else:
		call(['./fasttext supervised -input trainvalphrases_fb.txt -output model_fb'],shell=True)
...
if os.path.isfile('fasttext_confusion.matrix') and os.path.isfile('fasttext_prediction.time'):
	with open('fasttext_prediction.time','r') as ftime:
		for line in ftime:
			testtime = float(line)
		ftime.close()
		
	cmat = loadCmatFromFile('fasttext_confusion.matrix')
else:
	start_time = time.time()

	p = Popen(['./fasttext predict model_fb.bin testphrases_fb.txt 1'],shell=True,stdin=PIPE,stdout=PIPE,stderr=PIPE)
	...
	saveCmatToFile('fasttext_confusion.matrix',cmat)
	
	with open('fasttext_prediction.time','w') as ftime:
		ftime.write('%f\n' % testtime)
		ftime.close()
