...
def make_wordlevel(sentence,w2vmodel,vsize,convh):
	result = []
	
	words = sentence.split(' ')
	
	if len(words) < convh:
		prelen = int(math.ceil((convh-len(words))/2))
		trlen = convh-len(words)-prelen
...
if VALIDATION:
	cond = os.path.isfile('deep_cnn.model') and os.path.isfile('deep_cnn_val.acc')
else:
	cond = os.path.isfile('deep_cnn.model')

if cond:
	model = torch.load('deep_cnn.model')
	
	if VALIDATION:
		with open('deep_cnn_val.acc','r') as dval:
			for line in dval:
				valacc = float(line)
			dval.close()
else:
	...
	if VALIDATION:
		...
		with open('deep_cnn_val.acc','w') as dval:
			dval.write('%f\n' % valacc)
			dval.close()
	
	torch.save(model,'deep_cnn.model')
		
testtime = 0
		
if os.path.isfile('deep_cnn_confusion.matrix') and os.path.isfile('deep_cnn_prediction.time'):
	cmat = loadCmatFromFile('deep_cnn_confusion.matrix')
	
	with open('deep_cnn_prediction.time','r') as dtime:
		for line in dtime:
			testtime = float(line)
		dtime.close()
else:
	...
	saveCmatToFile('deep_cnn_confusion.matrix',cmat)
	
	with open('deep_cnn_prediction.time','w') as dtime:
		dtime.write('%f\n' % testtime)
		dtime.close()
